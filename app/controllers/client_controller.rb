...
begin
    #@counter=User.find(:first,:conditions=>["login=?",@@session['user']['login']])
     @counter=User.find(:first,:conditions=>["login=?",@session['user']])
     @x=Tokendisplay.update(@counter.counterno,{:tokenno=>'0000',:calledtime=>'00:00:00'}) 
     render :update do |page|
...
begin
  ...
  if @transact.missingflag.to_i<@set.missingcount.to_i
    if @transact.redirecttime==nil
      ...
      b.tokenstatus=3
      b.flag='N'
      b.missingflag=b.missingflag.to_i+1
